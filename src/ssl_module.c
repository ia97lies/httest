...
static apr_status_t worker_ssl_ctx_p12(worker_t * worker, const char *infile, 
  ...
  if (!(in = BIO_new_file(infile, "rb"))) {
    worker_log(worker, LOG_ERR, "Could not open p12 \"%s\"", infile);
    return APR_EINVAL;
  }
...
static apr_status_t block_SSL_CONNECT(worker_t * worker, worker_t *parent, apr_pool_t *ptmp) {
  ...
  if (!sslstr) {
    worker_log(worker, LOG_ERR, "Missing type, must be one of SSL|SSL2|SSL3|TLS1");
    return APR_EGENERAL;
  }
  ...
  config->sess = NULL;
    ...
    worker_log(worker, LOG_ERR, "Can not do a SSL connect, cause no TCP connection available");
    return APR_EGENERAL;
  }
  return APR_SUCCESS;
}
...
static apr_status_t block_SSL_GET_SESSION(worker_t * worker, worker_t *parent, apr_pool_t *ptmp) {
  ...
  if (!copy) {
    worker_log(worker, LOG_ERR, "Missing varibale name to store session in");
    return APR_EGENERAL;
  }

  if (!worker->socket || !worker->socket->socket || !worker->socket->is_ssl) {
    worker_log(worker, LOG_ERR, "No established ssl socket");
...
static apr_status_t block_SSL_GET_SESSION_ID(worker_t * worker, worker_t *parent, apr_pool_t *ptmp) {
  ...
  if (!worker->socket || !sconfig->ssl) {
    worker_log(worker, LOG_ERR, "Need an ssl connection");
    return APR_ENOSOCKET;
  }
...
static apr_status_t block_SSL_GET_CERT_VALUE(worker_t * worker, worker_t *parent, apr_pool_t *ptmp) {
  ...
  if (!cmd) {
    worker_log(worker, LOG_ERR, "SSL variable name is missing");
    return APR_EGENERAL;
  }

  if (!var) {
    worker_log(worker, LOG_ERR, "variable name to store result is missing");
    return APR_EGENERAL;
  }
...
static apr_status_t block_SSL_LOAD_CERT(worker_t * worker, worker_t *parent, apr_pool_t *ptmp) {
  ...
  if (!config->cert) {
    worker_log(worker, LOG_ERR, "Not a valid cert (PEM)");
    return APR_EINVAL;
  }
  return APR_SUCCESS;
}
...
static apr_status_t block_SSL_SET_CERT(worker_t * worker, worker_t *parent, apr_pool_t *ptmp) {
  ...
  /* else set cert */
  if (!config || !config->cert) {
    worker_log(worker, LOG_ERR, "No cert to use, get a cert with _SSL:LOAD_CERT");
    return APR_EINVAL;
  }

  if (SSL_CTX_use_certificate(config->ssl_ctx, config->cert) <=0) {
    worker_log(worker, LOG_ERR, "Can not use this cert");
    return APR_EINVAL;
  }
  return APR_SUCCESS;
}
...
static apr_status_t ssl_client_port_args(worker_t *worker, char *portinfo, 
  ...
  if (!worker->socket) {
    worker_log(worker, LOG_ERR, "No socket available");
    return APR_ENOSOCKET;
  }
