...
int main(int argc, const char *const argv[]) {
  ...
  htt = _test_reset();
  fprintf(stdout, "function resolve with parameters... ");
  {
    apr_status_t status;
    char *buf = apr_pstrdup(pool, 
        "function foo a b : r\n\
    ...
    status = htt_run(htt);
    assert(status == APR_SUCCESS);
    assert(strcmp(global_buf, "hallo welt bar\n") == 0);
  }
  fprintf(stdout, "ok\n");
  ...
  htt = _test_reset();
  fprintf(stdout, "req expect wait with vars -> ok... ");
  {
    apr_status_t status;
    char *buf = apr_pstrdup(pool, 
        "set bar=foobar\n\
  ...
  htt = _test_reset();
  fprintf(stdout, "req 3 filters expect wait -> ok... ");
  {
    apr_status_t status;
    char *buf = apr_pstrdup(pool, 
        "function foo2bar in : out\n\
