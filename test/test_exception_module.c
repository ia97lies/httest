...
int main(int argc, const char *const argv[]) {
  ...
  fflush(stdout);
  {
    ...
    status = htt_run(htt);
    assert(status == APR_SUCCESS);
    bufreader = htt_bufreader_buf_new(pool, global_buf[0], strlen(global_buf[0]));
    status = htt_bufreader_read_line(bufreader, &line);
    status = htt_bufreader_read_line(bufreader, &line);
    status = htt_bufreader_read_line(bufreader, &line);
    assert(status == APR_SUCCESS);
    assert(strcmp(line, "finally") == 0);
  }
  fprintf(stdout, "ok\n");

  htt = _test_reset();
  fprintf(stdout, "finally on error... ");
  fflush(stdout);
  {
    apr_status_t status;
    htt_bufreader_t *bufreader;
